...
class PlottingGraphs(GraphScene):
	...
	def construct(self):
		...
		title=TextMobject(r"Seeing the Addition of two Complex Numbers \\ in Complex Plane")
		...
		v_text1=TextMobject(r"$=6+9i$").scale(.8)
		v1_text=(v1_text.next_to(v1)).scale(.8)
		v2_text=(v2_text.next_to(v2)).scale(.8)
		v_text=v_text.shift(1.3*v.get_corner(UP+RIGHT)).scale(.8)
		...
		self.play(ShowCreation(v1_text),ShowCreation(v1_lx),ShowCreation(v1_ly))
		self.wait(.5)

		self.play(ShowCreation(v2))
		self.play(ShowCreation(v2_text),ShowCreation(v2_lx),ShowCreation(v2_ly))
		self.wait(.5)
		
		self.play(ApplyMethod(z_2.shift,a1))
		self.play(ApplyMethod(v1_lx.shift,5*Y) , ApplyMethod(v1_ly.shift,2*X), ApplyMethod(v1_text.shift,2*X))
		self.wait(1)

		self.play(ShowCreation(v))
		self.play(ShowCreation(v_text))
		self.play(ShowCreation(v_text1),Transform(VGroup(v1_lx,v2_lx),v_lx), Transform(VGroup(v1_ly,v2_ly),v_ly))
		self.wait(3)
